...
2. **Fill in the values:** edit `.env` so that `PROJECT_NAME`, `HOST_USERNAME`, `HOST_UID`, `HOST_GID`, `WORKSPACE_FOLDER`, `LOCALE`, Git identity, `EDITOR_CHOICE`, resource limits, and (optionally) workflow-specific names like `CONTAINER_HOSTNAME_*` / `DOCKER_IMAGE_NAME` match your machine. This file is the single source of truth.
